<form id="form" method="post" action="/otherobjects/form/" onsubmit="return ooSubmitForm('${blockName}','${(blockData.id)!}',${blockGlobal?string("true", "false")})">
<#if blockData??>
	<#-- Existing block -->
...
<#else> 
	...
	<input type="hidden" class="hidden" name="code" value="${blockName}" />
