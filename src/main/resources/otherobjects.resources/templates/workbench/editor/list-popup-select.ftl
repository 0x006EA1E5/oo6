...
<#if viewMode == "thumbnails">
	...
	<#if item_index % 5 == 0></tr><tr></#if>
		<td class="oo-image">
	   		...
	   		<a href="javascript:window.opener.ooPopupInlineSelect('${RequestParameters.fieldName}','${item.wikiCode}');window.close();">
...
<#else>
	...
	<#if ! item.class.name?ends_with("Folder") && ! item.class.name?ends_with("PublishingOptions")>  <!-- FIXME -->
	 <tr>
		...
		<td class="oo-action"><#if item.linkPath??>
		<#-- <a href="javascript:window.opener.ooPopupSelect('${RequestParameters.fieldName}','${item.editableId}','${item.ooLabel}','${oo.resourceUrl(item.ooIcon)}','');window.close();">-->
		<a href="javascript:window.opener.ooPopupInlineSelect('${RequestParameters.fieldName}','${item.wikiCode}');window.close();">
		Select</a></#if></td>
	</tr>
	</#if>
