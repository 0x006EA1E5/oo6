...
public class GlobalInfoBean implements InitializingBean,
	...
	private void initialiseJndiProperties() {
		...
		try {
			jndiProperties.put(SERVER_NAME_KEY, (String)jndiTemplate.lookup("java:comp/env/" + JNDI_SERVER_NAME_PATH));
		} catch (NamingException e) {
			logger.warn("Couldn't read jndi value for name: " + JNDI_SERVER_NAME_PATH);
		}
		
		try {
			jndiProperties.put(CONTEXT_PATH_KEY, (String)jndiTemplate.lookup("java:comp/env/" + JNDI_CONTEXT_PATH_PATH));
		} catch (NamingException e) {
