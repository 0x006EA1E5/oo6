    ...
    private ModelAndView generateNavigatorData(HttpServletRequest request)
    {
        ...
        for (BaseNode dynaNode : contents)
        {
            if (dynaNode instanceof Folder)
            {
                ...
                n1.put("allAllowedTypes", dynaNode.getPropertyValue("allAllowedTypes"));
                ...
                if (!StringUtils.isEmpty((String) dynaNode.getPropertyValue("cssClass")))
                {
                    n1.put("cls", dynaNode.getPropertyValue("cssClass") + "-nav-item");
